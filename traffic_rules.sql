...
/* -- all questions --*/
SELECT COUNT(DISTINCT QUESTION_ID) FROM TEST t
  LEFT JOIN TEST_SECTION ts
    ON t.ID = ts.TEST_ID
  LEFT JOIN TEST_SECTION_QUESTION_XREF tsqx
    ON ts.ID = tsqx.TEST_SECTION_ID
...
SELECT SUM(IS_CORRECT) as CORRECTLY_ANSWERED_COUNT, COUNT(*) as ANSWERED_COUNT, MIN(DATETIME) as STARTED, MAX(DATETIME) as ENDED
FROM TEST_RESULT_ANSWERS_FACTS
WHERE STUDENT_ID = 4 AND TEST_ID = 1;
